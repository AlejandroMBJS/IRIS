 ...
 * @description Supervisor Dashboard - View direct reports, pending approvals, team performance
 *
 * USER PERSPECTIVE:
 *   - View team members (direct reports only)
 *   - See pending absence requests requiring approval
 *   - Quick access to approval actions
 *   - Team attendance and performance metrics
 *
 * DEVELOPER GUIDELINES:
 *   OK to modify: Add team performance metrics, customize widgets
 *   CAUTION: Only shows direct reports (filtered by supervisor_id)
 *   DO NOT modify: Access control - supervisor role only
 */
...
import {
  ...
  TrendingUp, UserCheck, ChevronRight, FileText
} from "lucide-react"
import { Button } from "@/components/ui/button"
import { DashboardLayout } from "@/components/layout/dashboard-layout"
import { isAuthenticated, getCurrentUser, isSupervisor } from "@/lib/auth"
...
export default function SupervisorDashboardPage() {
  ...
  async function loadDashboardData() {
    try {
      // Get employees - backend automatically filters to show only direct reports
      ...
      <div className="space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="group relative bg-gradient-to-br from-slate-800/80 to-slate-900/80 backdrop-blur-xl rounded-2xl p-6 border border-slate-700/50 hover:border-emerald-500/50 transition-all duration-300 hover:shadow-xl hover:shadow-emerald-500/10">
            <div className="flex items-start justify-between">
              <div className="space-y-3">
                <p className="text-slate-400 text-sm font-medium uppercase tracking-wider">Quick Access</p>
                <div className="space-y-2">
                  <button
                    onClick={() => router.push("/approvals")}
                    className="w-full text-left px-3 py-2 bg-slate-800/50 hover:bg-slate-700/50 rounded-lg text-sm text-slate-300 hover:text-white transition-colors"
                  >
                    → Approvals
                  </button>
                  <button
                ...
                {employees.length === 0 ? (
                  <tr>
                    <td colSpan={5} className="px-6 py-12 text-center">
                      ...
                      <p className="text-slate-400">No employees assigned</p>
                      <p className="text-sm text-slate-500 mt-2">Employees are assigned when you are their supervisor</p>
                    </td>
                  </tr>
                ) : (
                  employees.map((emp) => (
        ...
        <div className="bg-gradient-to-br from-slate-800/60 to-slate-900/60 backdrop-blur-xl rounded-2xl border border-slate-700/50 p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            >
              ...
              <span className="text-white font-medium">Approve Requests</span>
              <span className="text-xs text-slate-400 mt-1">Review pending</span>
            </button>
            ...
            >
              ...
              <span className="text-white font-medium">View Team</span>
              <span className="text-xs text-slate-400 mt-1">Manage employees</span>
            </button>
            ...
            >
              ...
              <span className="text-xs text-slate-400 mt-1">View records</span>
            </button>
          </div>
        </div>
      </div>
